...
def pso(nvar, ncal, type, function, chi, w):
    ...
    class Problem:
        ...
        def __init__(self, nvar, ncal, type, function, chi, w):
            ...
            self.function = function     # (MatrixTest)
            #
            ...
            print("Data: ")
            print(self.data)  # Dada retieved from file
            # ----------------------------------------------------------------------------------------------------------

        def cost_function(self, x):
            #
            # ============== Matrix Test ===============================================================================
            if self.function == 'MatrixTest':
                a = x.round()
                a = -a
                return np.sum(a)
            #
    ...
    class Particle(Problem):
        ...
        def __init__(self):
            ...
            self.position = np.random.rand(self.dimension, self.dimension) * \
            ...
            for i in range(self.dimension):
                for j in range(self.dimension):
                    ...
                    while flag is False:    # make the particle bounce
                        if self.position[i][j] > self.upper_bound:
                            self.position[i][j] = self.upper_bound - (self.position[i][j] - self.upper_bound)
                        if self.position[i][j] < self.lower_bound:
                            self.position[i][j] = self.lower_bound - (self.position[i][j] - self.lower_bound)
                        if self.lower_bound < self.position[i][j] < self.upper_bound:
                            flag = True
    ...
    class PSO(Problem):
        ...
        def __init__(self):
            Problem.__init__(self, nvar, ncal, type, function, chi, w)
            self.swarm = np.zeros([self.swarm_size, self.dimension, self.dimension])
            self.fitness = np.zeros(self.swarm_size) + np.inf
            self.velocity = np.zeros([self.swarm_size, self.dimension, self.dimension])
            self.personal_best = np.zeros(self.swarm_size) + np.inf
            self.personal_best_pos = np.zeros([self.swarm_size, self.dimension, self.dimension])
            if self.type_PSO == 'Global':
                self.global_best = np.inf
                self.global_best_pos = np.zeros([self.dimension, self.dimension])
            elif self.type_PSO == 'Local':
                self.global_best = np.zeros(self.swarm_size) + np.inf
                self.global_best_pos = np.zeros([self.swarm_size, self.dimension, self.dimension])
            #
            # initialize swarm
...
f, x = pso(nvar=5, ncal=1000, type='Global', function='Cancer', chi=0.9, w=0.9)
